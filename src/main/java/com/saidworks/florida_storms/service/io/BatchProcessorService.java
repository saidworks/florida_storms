...
public class BatchProcessorService {
    ...
    public CompletableFuture<ProcessedBatch> processBatch(RawBatch rawBatch) {
        ...
        log.debug("Processing batch {} (lines {}-{})", 
                  rawBatch.getBatchId(), rawBatch.getStartLineNumber(), rawBatch.getEndLineNumber());

        ProcessedBatch.ProcessedBatchBuilder batchBuilder = ProcessedBatch.createEmpty(rawBatch.getBatchId());
        ...
        for (int i = 0; i < rawBatch.getLines().size(); i++) {
            ...
            try {
                if (line.startsWith("AL")) {
                    ...
                    currentPartial = ProcessedBatch.PartialCyclone.builder()
                ...
                String error = String.format("Error parsing line %d: %s - %s", 
        ...
        ProcessedBatch result = batchBuilder
        ...
        log.debug("Completed batch {} in {}ms with {} partial cyclones", 
                  rawBatch.getBatchId(), processingTime, partialCyclones.size());
    ...
    public boolean validateBatch(ProcessedBatch batch) {
        if (!batch.isValid()) {
            log.warn("Batch {} has parsing errors: {}", batch.getBatchId(), batch.getValidationErrors());
            return false;
        }
        ...
        for (ProcessedBatch.PartialCyclone partial : batch.getPartialCyclones()) {
            if (partial.isHeaderPresent() && partial.getDataLines().isEmpty()) {
                log.warn("Batch {}: Cyclone {} has header but no data lines possible missing data verify end result",
                         batch.getBatchId(), partial.getCycloneId());
            }
        }
